...
dss_mem_stats_init(struct mem_stats *stats, int xs_id)
{
	...
	rc = d_tm_add_metric(&stats->ms_total_usage, D_TM_COUNTER, "Total memory usage",
			     "byte", "mem/total_mem/xs_%u", xs_id);
	if (rc)
...
dss_mem_total_alloc_track(void *arg, daos_size_t bytes)
{
	...
	d_tm_inc_counter(stats->ms_total_usage, bytes);
}
...
dss_mem_total_free_track(void *arg, daos_size_t bytes)
{
	...
	d_tm_dec_counter(stats->ms_total_usage, bytes);
}
