...
class FtestTagMap():
    ...
    def minimal_tags(self, path):
        ...
        classes = self.__mapping[file_path]
...
def read_tag_config():
    ...
    for key, val in config.items():
        ...
        extra_keys = set(config[key].keys()) - set(['tags', 'handler', 'exclusive'])
        if extra_keys:
        ...
        # Set default exclusive and check for invalid values
        if 'exclusive' not in config[key]:
            config[key]['exclusive'] = False
        elif not isinstance(config[key]['exclusive'], bool):
            raise ValueError(f'Invalid exclusive: {config[key]["exclusive"]}')
