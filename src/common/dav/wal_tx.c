...
{
	struct umem_wal_tx *utx = hdl->do_umem_wtx;
	int rc;

	hdl->do_umem_wtx = dav_umem_wtx_new(hdl);
	if (hdl->do_umem_wtx == NULL)
		FATAL("Failed to create new umem wal tx instance\n");
...
dav_wal_tx_snap(void *hdl, void *addr, daos_size_t size, void *src, uint32_t flags)
{
	...
	struct dav_tx		*tx = utx2wtx(dav_hdl->do_umem_wtx);
